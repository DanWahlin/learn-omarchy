...
In the course app, this entrance belongs to **Welcome to Omarchy**, lesson #1, not the tour lesson. It starts automatically after first-time coach selection and can be replayed from the lesson list without resetting progress. The pack scene finishes, the host flies the coach to the center for a greeting, then points to the highlighted app toolbar for a brief explanation of Settings, Keys, Mute, and Exit. The outline follows the actual toolbar bounds, including wrapped rows. Finally, the host reveals the menu and points at **Omarchy tour**, lesson #2. Captions and narration wait for arrival. The menu recommends the tour without starting it; any lesson remains available. The lab can still preview/replay the pack sequence independently.

The app persists `welcomeSeen` when onboarding begins. Absent flags migrate from the legacy `tourSeen` marker or any recorded lesson/activity/bookmark progress; an explicit false flag requests a fresh welcome. Confirmed reset clears that flag and the coach choice. The small settings and progress files use blocking atomic writes; reset reports success only after both writes finish without a save error. Failed saves are logged and shown in Settings instead of displaying a successful reset. Done leads through coach selection before replaying the welcome (an explicit character override keeps its selected pack). Skipping the entire welcome, opening Settings, changing packs, selecting a lesson, or closing cancels pending playback, reading timers, and deferred handoffs. The new greeting/recommendation use plain text and talking animation: existing tour recordings announce a tour and are deliberately not reused here.
